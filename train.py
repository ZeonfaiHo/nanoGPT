...
if init_from == 'resume':
    try:
        ...
        print('WARNING: load optimizer failed! check whether model architecture matches')
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        aux_lossf = aux_loss.item() * gradient_accumulation_steps
        if local_iter_num >= 5: # let the training loop settle a bit
            mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)
            running_mfu = mfu if running_mfu == -1.0 else 0.9*running_mfu + 0.1*mfu
        print(f"iter {iter_num}: loss {lossf:.4f}, auxilary loss {aux_lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
    iter_num += 1
    local_iter_num += 1
